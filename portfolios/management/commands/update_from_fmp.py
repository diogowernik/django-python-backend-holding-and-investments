...
import pandas as pd
from investments.utils.common import get_app_df, merge_dataframes, get_usd_to_brl_today, update_investment, update_ranking
...
class Command(BaseCommand):
    def handle(self, *args, **options):
        ...
        def create_df_from_api(url, columns, ticker_data_map):
        ...
        ratios_df = create_df_from_api('https://financialmodelingprep.com/api/v3/ratios-ttm',
                            ...
                            'ffo': lambda data: float(data['operatingCashFlowPerShareTTM'])})
        print('ratios')
        print(ratios_df)
        ...
        profile_df = create_df_from_api('https://financialmodelingprep.com/api/v3/profile',
                            ...
                            {'price': lambda data: float(data['price']),
                                ...
                                'market_cap': lambda data: float(data['mktCap'])})
        print('profile')
        print(profile_df)
        ...
        # update the database
        # update_investment(Reit, merged_df, ['twelve_m_dividend', 'der', 'ffo', 'p_ffo', 'p_vpa', 'roic', 'earnings_yield', 'price_usd', 'price_brl', 'ffo_yield'])
        update_investment(Stocks, merged_df, ['twelve_m_dividend', 'der', 'ffo', 'p_ffo', 'p_vpa', 'roic', 'earnings_yield', 'price_usd', 'price_brl', 'ffo_yield'])
