...
def update_asset_historical_prices(asset_ticker):
    ...
    data = yf.download(asset_ticker, start=start_date.strftime('%Y-%m-%d'), end=datetime.today().strftime('%Y-%m-%d'))
...
class Command(BaseCommand):
    def handle(self, *args, **options):
        update_asset_historical_prices('AAPL')
        update_asset_historical_prices('MSFT')
